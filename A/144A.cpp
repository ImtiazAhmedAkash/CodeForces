...
int main()
{
     ...
     * and the minimum needs will be moved to the n - 1 index. The only can move one index at one step.
     * 
     * So, first we need to find the first position of the max element (important if the max is present twice).
     * That's why we used only greater than sign.
     ...
     * 
     * If the max element is present after min element, then we will increment the min index by 1.
     * Because when we will swap two elements to bring the max to the front, the min will move to the next index from its current index.
     * 
